...
if (!DATABASE_URL) {
  ...
  if (process.env.SKIP_IF_NO_URL === '1') {
    console.log(
      `Secret ${secretName} ausente. Pulando sincronização Lotofácil. Cadastre-o em Settings → Secrets and variables → Actions.`,
    )
    process.exit(0)
  }
  console.error(`Secret ${secretName} ausente. Use: pnpm db:sync-lotofacil`)
  process.exit(1)
}
...
function normalizar(dados) {
  ...
  if (!numero || dezenas.length !== 15 || !data_sorteio) return null
...
async function gravarLotes(sql, rows) {
  ...
  for (let i = 0; i < rows.length; i += BATCH) {
    ...
    await sql.query(
      `INSERT INTO public.concursos_lotofacil (numero, data_sorteio, dezenas)
       SELECT
         (r->>'numero')::int,
         (r->>'data_sorteio')::date,
...
console.log('Garantindo schema public.concursos_lotofacil...')
await garantirSchema(sql)

console.log('Buscando último concurso oficial da Lotofácil (Caixa)...')
const ultimo = normalizar(await fetchJson(`${CAIXA}/`))
if (!ultimo) {
  console.error('Não foi possível ler o último concurso da Lotofácil na Caixa.')
  process.exit(1)
}
...
if (full) {
  ...
  const faltando = await sql`
    ...
    WHERE NOT EXISTS (
      SELECT 1 FROM public.concursos_lotofacil c WHERE c.numero = gs
    )
  `
...
if (rows.length === 0) {
  console.log('Nada novo para gravar (Lotofácil).')
  process.exit(0)
}

console.log(`${rows.length} concursos Lotofácil baixados. Gravando no Neon...`)
await gravarLotes(sql, rows.sort((a, b) => a.numero - b.numero))

const [{ total }] = await sql`SELECT COUNT(*)::int AS total FROM public.concursos_lotofacil`
console.log(`Sincronização Lotofácil concluída. Concursos no Neon: ${total}`)
